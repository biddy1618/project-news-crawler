'''
Module helper with additional functions.
...
def _message(msg: str, e: Exception = None) -> str:
    ...
    '''
    if e:
        return f'{msg}. {type(e).__name__}: "{e}".'
    else:
        return f'{msg}'
...
def generate_dates(start_date: str, end_date: str = None) -> List[str]:
    ...
    try:
        ...
        end_date = datetime.datetime.strptime(end_date, format) if end_date is not None \
            else start_date + datetime.timedelta(days=1)
    except ValueError as e:
        logger.error(_message(
            'Invalid date format: {start_date}, {end_date}. Input the date in following format: "dd.mm.yyyy"', e))
        # raise SystemExit(e)
        return None

    if start_date >= end_date:
        logger.error(_message('Make sure that the end date is after the start date'))
        # raise SystemExit()
        return None

    date_generated = [
        (start_date + datetime.timedelta(days=x)).strftime(format) for x in range(0, (end_date-start_date).days)]
